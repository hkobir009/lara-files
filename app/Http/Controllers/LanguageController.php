...
class LanguageController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $language = Language::findOrFail($id);
        if (config('custom.default_language') == $language->code && config('custom.default_language') != $request->code) {
    ...
    {
        $language = Language::findOrFail($request->id);
        if($language->code == config('custom.default_language') && $request->status == 0) {
    ...
    {
        $language = Language::findOrFail($id);
        if (config('custom.default_language') == $language->code) {
            ...
            if($language->code == Session::get('locale')){
                Session::put('locale', config('custom.default_language'));
            }
            Language::destroy($id);
