                ...
                <div class="col-lg-6 col-xl-5 mx-auto">
                    <div class="card">
                        <div class="text-center pt-5">
                            ...
                            @if(!is_null(auth()->user()->resent_verification_expires_at) && now()->lessThan(auth()->user()->resent_verification_expires_at))
                                <div id="countdown">
                                    <span>Resent after</span>
                                    <h3 class="values"></h3>
                                </div>
                            @else
...
@section('script')
    <script>
        ...
        timer.start({
            countdown: true,
            startValues: {seconds: '{{ now()->diffInSeconds(auth()->user()->resent_verification_expires_at) }}'}
        });
        ...
        timer.addEventListener('targetAchieved', function (e) {
            $('#countdown .values').html('<a href="{{ route('verification.phone.resend') }}" class="btn btn-link">{{translate('Resend Code')}}</a>');
        });
    </script>
@endsection
